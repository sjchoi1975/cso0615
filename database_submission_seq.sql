...
-- 3. 이용자용 뷰 (제출 순서별 그룹화)
CREATE OR REPLACE VIEW edi_list_user_view AS
SELECT 
    -- 가상의 submission_id 생성
    ...
    MAX(grouped.memo) as memo,
    -- 같은 순서의 모든 파일이 확인되었는지 체크
    BOOL_AND(grouped.confirm) as confirm,
    MIN(grouped.created_at) as created_at,
    MAX(grouped.updated_at) as updated_at,
    COUNT(*) as file_count,
    -- 제약사 개수 (중복 제거)
    ...
    )) FILTER (WHERE pc.id IS NOT NULL) as companies
...
-- 4. 관리자용 뷰에 submission_seq 추가
CREATE OR REPLACE VIEW edi_list_admin_view AS
SELECT 
    ef.id,
...
-- 6. 인덱스 추가
CREATE INDEX IF NOT EXISTS idx_edi_files_submission_seq 
ON edi_files(settlement_month_id, member_id, hospital_id, submission_seq); 